...
(function() {
  ...
  _.each = function(collection, iterator) {
    // Determine if collection is Array/Obj
    // let isArray = Array.isArray(collection);
    // If no iterator provided, set iterator as identity
    if (!iterator) {
      let iterator = _.identity(collection);
    }
  ...
  _.map = function(collection, iterator) {
    ...
    var mappedArr = [];
    // If no iterator _.identity()
    if (!iterator) { iterator = _.identity(collection); }
    _.each(collection, function(item) {
  ...
  _.zip = function() {
    ...
    //figure out longest arg and work from there
    ...
    for (var i = 0; i < longest; i++) {
      ...
      for (var j = 0; j < arguments.length; j++) {
        if (arguments[j][i] === undefined) {
          zipper.push(undefined);
        } else {
          zipper.push(arguments[j][i]);
        }
      }
      zippedArray.push(zipper);
    }
    return zippedArray;
  };
